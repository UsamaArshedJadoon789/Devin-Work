...
void	list_path_setup(t_data *data)
{
	...
	{
		if (access(data->list_path[i], F_OK) == -1)
			printf("\033[0;31mWARNING : '%s' does not exist\033[0;0m\n", \
				data->list_path[i]);
		else if (access(data->list_path[i], X_OK) == -1)
			printf("\033[0;31mWARNING : '%s' is not accessible\033[0;0m\n", \
				data->list_path[i]);
		i++;
	}
}
